...
public void mouseClicked(MouseEvent e) {
    ...
    if (e.getX() >= 920 && e.getX() <= 970 && e.getY() >= 20 && e.getY() <= 60) {
        if (playpause) {
            ...
            playpause = true;
            time.start();
            synchronized (stage) { stage.notify(); }
            synchronized (showitem) { showitem.notify(); }
            synchronized (actor) { actor.notify(); }
            synchronized (enm) { enm.notify(); }
            synchronized (ensky) { ensky.notify(); }
            synchronized (enspcial) { enspcial.notify(); }
        }
        repaint();
    }
